...
disease_results <- function(replicates,
  ...
  separated_segment_indices <- NULL
  if (!is.null(result_stages) & !is.null(result_compartments)) {
    result_segments <- result_stages * result_compartments
    # Calculate segment indices
    selected_indices <- expand.grid(result_stage_indices, result_compartment_indices)
    segment_indices <- sort((selected_indices$Var1 - 1) * result_compartment_length +
      selected_indices$Var2)

    if (length(unique(result_segments[segment_indices])) > 1) {
      # Separate segment combinations for abundance and harvest results
      separated_segment_indices <- lapply(
        unique(segment_indices),
        function(s) which(segment_indices == s)
      )
      names(separated_segment_indices) <- lapply(
        separated_segment_indices,
        function(s) {
          paste(unique(names(s)),
            collapse = "."
          )
        }
      )
  ...
  result_segment_length <- length(segment_indices)
  ...
  initialize_attributes <- function() {
    ...
    if ("abundance" %in% results_selection) {
      ...
      if (replicates > 1) {
        if ("replicate" %in% results_selection) {
          results$abundance <- array(0, c(populations, time_steps, seasons, replicates))
      ...
      if (length(separated_segment_indices)) {
        results$abundance_segments <- lapply(
          separated_segment_indices,
          function(s) results$abundance
        )
        results$all$abundance_segments <- lapply(
          separated_segment_indices,
        ...
        results$abundance_compartments <- lapply(
          separated_compartment_indices,
          function(s) results$abundance
        )
        results$all$abundance_compartments <- lapply(
          separated_compartment_indices,
          function(s) results$all$abundance
        )
      }
    ...
    if ("harvested" %in% results_selection) {
      ...
      if (replicates > 1) {
        if ("replicate" %in% results_selection) {
          results$harvested <- array(0, c(
            populations, time_steps, seasons,
            replicates
          ))
      ...
      if (length(separated_segment_indices)) {
        results$harvested_segments <- lapply(
          separated_segment_indices,
          function(s) results$harvested
        )
        results$all$harvested_segments <- lapply(
          segment_indices,
        ...
        results$harvested_compartments <- lapply(
          separated_compartment_indices,
          function(s) results$harvested
        )
        results$all$harvested_compartments <- lapply(
          separated_compartment_indices,
          function(s) results$all$harvested
        )
      }
  ...
  calculate_at_season <- function(r, tm, season, segment_abundance, harvested, results) {
    ...
    if (!is.null(segment_abundance)) { # results calculated from abundance

      # Select abundance counts for included stages
      abundance_count <- as.array(.colSums(initial_abundance[segment_indices, ],
        m = result_segment_length,
        n = populations
      ))
      ...
      if (length(separated_stage_indices) && length(separated_compartment_indices)) {
        abundance_segments_count <- lapply(separated_segment_indices, function(s) {
          as.array(.colSums(segment_abundance[s, ], m = length(s), n = populations))
        })
        all_abundance_segments_count <- lapply(abundance_segments_count, sum)
      } else if (length(separated_stage_indices)) {
        abundance_stages_count <- lapply(separated_stage_indices, function(s) {
          stage_indices <- seq(s, nrow(segment_abundance),
            by = result_stage_length
          )
          as.array(.colSums(segment_abundance[stage_indices, ],
            m = length(stage_indices), n = populations
          ))
        })
        all_abundance_stages_count <- lapply(abundance_stages_count, sum)
      } else if (length(separated_compartment_indices)) {
        abundance_compartments_count <- lapply(separated_compartment_indices, function(s) {
          compartment_indices <- seq(s, nrow(segment_abundance),
            by = result_compartment_length
          )
          as.array(.colSums(segment_abundance[compartment_indices, ],
            m = length(compartment_indices), n = populations
          ))
        })
        all_abundance_compartments_count <- lapply(abundance_compartments_count, sum)
      }
      ...
      if ("abundance" %in% results_selection) {
        if (replicates > 1) {
          if ("replicate" %in% results_selection) {
            ...
            results$all$abundance[tm, season, r] <- all_abundance_count
          } else { # summarize (default)
            previous_abundance_mean <- results$abundance$mean[, tm, season]
            results$abundance$mean[, tm, season] <- previous_abundance_mean +
              (abundance_count - previous_abundance_mean) / r
            previous_abundance_var_by_r <- results$abundance$sd[, tm, season]
            results$abundance$sd[, tm, season] <- previous_abundance_var_by_r +
              (abundance_count - previous_abundance_mean) *
                (abundance_count - results$abundance$mean[, tm, season])

            if (r > 1) {
              results$abundance$min[, tm, season] <- pmin(
                results$abundance$min[, tm, season],
                abundance_count
              )
              results$abundance$max[, tm, season] <- pmax(
                results$abundance$max[, tm, season],
                abundance_count
              )
            } else {
            ...
            previous_all_abundance_mean <- results$all$abundance$mean[tm, season]
            results$all$abundance$mean[tm, season] <- previous_all_abundance_mean +
              (all_abundance_count - previous_all_abundance_mean) / r
            previous_all_abundance_var_by_r <- results$all$abundance$sd[tm, season]
            results$all$abundance$sd[tm, season] <- previous_all_abundance_var_by_r +
              (all_abundance_count - previous_all_abundance_mean) *
                (all_abundance_count - results$all$abundance$mean[tm, season])
            results$all$abundance$min[tm, season] <- ifelse(r > 1,
              min(
                results$all$abundance$min[tm, season],
                all_abundance_count
              ),
              all_abundance_count
            )
            results$all$abundance$max[tm, season] <- ifelse(r > 1,
              max(
                results$all$abundance$max[tm, season],
                all_abundance_count
              ),
              all_abundance_count
            )
        ...
        if (length(separated_segment_indices)) {
          for (i in 1:length(separated_segment_indices)) {
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                results$abundance_segments[[i]][, tm, season, r] <- abundance_segments_count[[i]]
                results$all$abundance_segments[[i]][tm, season, r] <- all_abundance_segments_count[[i]]
              } else {
                previous_abundance_segments_mean <- results$abundance_segments[[i]]$mean[, tm, season]
                results$abundance_segments[[i]]$mean[, tm, season] <- previous_abundance_segments_mean +
                  (abundance_segments_count[[i]] - previous_abundance_segments_mean) / r
                previous_abundance_segments_var_by_r <- results$abundance_segments[[i]]$sd[, tm, season]
                results$abundance_segments[[i]]$sd[, tm, season] <- previous_abundance_segments_var_by_r +
                ...
                if (r == 1) {
                  results$abundance_segments[[i]]$min[, tm, season] <- abundance_segments_count[[i]]
                  results$abundance_segments[[i]]$max[, tm, season] <- abundance_segments_count[[i]]
                } else {
                  results$abundance_segments[[i]]$min[, tm, season] <- pmin(
                    results$abundance_segments[[i]]$min[, tm, season],
                    abundance_segments_count[[i]]
                  )
                  results$abundance_segments[[i]]$max[, tm, season] <- pmax(
                    results$abundance_segments[[i]]$max[, tm, season],
                    abundance_segments_count[[i]]
                  )
                }

                previous_all_abundance_segments_mean <- results$all$abundance_segments[[i]]$mean[tm, season]
                results$all$abundance_segments[[i]]$mean[tm, season] <- previous_all_abundance_segments_mean +
                  (all_abundance_segments_count[[i]] - previous_all_abundance_segments_mean) / r
                previous_all_abundance_segments_var_by_r <- results$all$abundance_segments[[i]]$sd[tm, season]
                results$all$abundance_segments[[i]]$sd[tm, season] <- previous_all_abundance_segments_var_by_r +
                ...
                results$all$abundance_segments[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$abundance_segments[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
            ...
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                ...
                if (r == 1) {
                  ...
                  results$abundance_stages[[i]]$min[, tm, season] <- pmin(
                    results$abundance_stages[[i]]$min[, tm, season],
                    abundance_stages_count[[i]]
                  )
                  results$abundance_stages[[i]]$max[, tm, season] <- pmax(
                    results$abundance_stages[[i]]$max[, tm, season],
                    abundance_stages_count[[i]]
                  )
                }
                ...
                results$all$abundance_stages[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$abundance_stages[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
        ...
        if (length(separated_compartment_indices)) {
          for (i in 1:length(separated_compartment_indices)) {
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                results$abundance_compartments[[i]][, tm, season, r] <- abundance_compartments_count[[i]]
                results$all$abundance_compartments[[i]][tm, season, r] <- all_abundance_compartments_count[[i]]
              } else {
                previous_abundance_compartments_mean <- results$abundance_compartments[[i]]$mean[, tm, season]
                results$abundance_compartments[[i]]$mean[, tm, season] <- previous_abundance_compartments_mean +
                  (abundance_compartments_count[[i]] - previous_abundance_compartments_mean) / r
                previous_abundance_compartments_var_by_r <- results$abundance_compartments[[i]]$sd[, tm, season]
                results$abundance_compartments[[i]]$sd[, tm, season] <- previous_abundance_compartments_var_by_r +
                ...
                if (r == 1) {
                  results$abundance_compartments[[i]]$min[, tm, season] <- abundance_compartments_count[[i]]
                  results$abundance_compartments[[i]]$max[, tm, season] <- abundance_compartments_count[[i]]
                } else {
                  ...
                  results$abundance_compartments[[i]]$max[, tm, season] <- pmax(
                    results$abundance_compartments[[i]]$max[, tm, season],
                    abundance_compartments_count[[i]]
                  )
                }

                previous_all_abundance_compartments_mean <- results$all$abundance_compartments[[i]]$mean[tm, season]
                results$all$abundance_compartments[[i]]$mean[tm, season] <- previous_all_abundance_compartments_mean +
                  (all_abundance_compartments_count[[i]] - previous_all_abundance_compartments_mean) / r
                previous_all_abundance_compartments_var_by_r <- results$all$abundance_compartments[[i]]$sd[tm, season]
                results$all$abundance_compartments[[i]]$sd[tm, season] <- previous_all_abundance_compartments_var_by_r +
                ...
                results$all$abundance_compartments[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$abundance_compartments[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
      ...
      # Extinction location results
      if ("extinction_location" %in% results_selection) {
        if (all_abundance_count > 0) { # => not extinct
      ...
      if ("occupancy" %in% results_selection) {
        if (replicates > 1) {
          if ("replicate" %in% results_selection) {
            results$all$occupancy[tm, season, r] <- sum(as.logical(abundance_count))
          } else { # summarize (default)
            occupancy_count <- sum(as.logical(abundance_count))
            previous_occupancy_mean <- results$all$occupancy$mean[tm, season]
            results$all$occupancy$mean[tm, season] <- previous_occupancy_mean +
              (occupancy_count - previous_occupancy_mean) / r
            previous_occupancy_var_by_r <- results$all$occupancy$sd[tm, season]
            results$all$occupancy$sd[tm, season] <- previous_occupancy_var_by_r +
              (occupancy_count - previous_occupancy_mean) *
                (occupancy_count - results$all$occupancy$mean[tm, season])
            results$all$occupancy$min[tm, season] <- ifelse(r == 1,
              occupancy_count,
              min(
                results$all$occupancy$min[tm, season],
                occupancy_count
              )
            )
            results$all$occupancy$max[tm, season] <- ifelse(r == 1,
              occupancy_count,
              max(
                results$all$occupancy$max[tm, season],
                occupancy_count
              )
            )
          }
    ...
    if (!is.null(harvested)) { # results calculated from harvested
      ...
      harvested_count <- as.array(.colSums(harvested[segment_indices, ],
        m = result_segment_length,
        n = populations
      ))
      all_harvested_count <- sum(harvested_count)
      if (length(separated_segment_indices)) {
        harvested_segments_count <- lapply(separated_segment_indices, function(s) {
          as.array(.colSums(harvested[s, ], m = length(s), n = populations))
        })
      } else if (length(separated_stage_indices)) {
        harvested_stages_count <- lapply(separated_stage_indices, function(s) {
          stage_indices <- seq(s, nrow(harvested), by = result_stage_length)
          as.array(.colSums(harvested[stage_indices, ], m = length(stage_indices), n = populations))
        })
      } else if (length(separated_compartment_indices)) {
        harvested_compartments_count <- lapply(separated_compartment_indices, function(s) {
          compartment_indices <- seq(s, nrow(harvested), by = result_compartment_length)
          as.array(.colSums(harvested[compartment_indices, ], m = length(compartment_indices), n = populations))
        })
      }
      ...
      if ("harvested" %in% results_selection) {
        if (replicates > 1) {
          if ("replicate" %in% results_selection) {
            ...
            results$all$harvested[tm, season, r] <- all_harvested_count
          } else { # summarize (default)
            previous_harvested_mean <- results$harvested$mean[, tm, season]
            results$harvested$mean[, tm, season] <- previous_harvested_mean +
              (harvested_count - previous_harvested_mean) / r
            previous_harvested_var_by_r <- results$harvested$sd[, tm, season]
            results$harvested$sd[, tm, season] <- previous_harvested_var_by_r +
            ...
            if (r == 1) {
              ...
              results$harvested$min[, tm, season] <- pmin(
                results$harvested$min[, tm, season],
                harvested_count
              )
              results$harvested$max[, tm, season] <- pmax(
                results$harvested$max[, tm, season],
                harvested_count
              )
            }

            previous_all_harvested_mean <- results$all$harvested$mean[tm, season]
            results$all$harvested$mean[tm, season] <- previous_all_harvested_mean +
              (all_harvested_count - previous_all_harvested_mean) / r
            previous_all_harvested_var_by_r <- results$all$harvested$sd[tm, season]
            results$all$harvested$sd[tm, season] <- previous_all_harvested_var_by_r +
              (all_harvested_count - previous_all_harvested_mean) *
                (all_harvested_count - results$all$harvested$mean[tm, season])
            results$all$harvested$min[tm, season] <- ifelse(r > 1,
              min(
                results$all$harvested$min[tm, season],
                all_harvested_count
              ),
              all_harvested_count
            )
            results$all$harvested$max[tm, season] <- ifelse(r > 1,
              max(
                results$all$harvested$max[tm, season],
                all_harvested_count
              ),
              all_harvested_count
            )
        ...
        if (length(separated_segment_indices)) {
          for (i in length(separated_segment_indices)) {
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                results$harvested_segments[[i]][, tm, season, r] <- harvested_segments_count[[i]]
                results$all$harvested_segments[[i]][tm, season, r] <- all_harvested_segments_count[[i]]
              } else {
                previous_harvested_segments_mean <- results$harvested_segments[[i]]$mean[, tm, season]
                results$harvested_segments[[i]]$mean[, tm, season] <- previous_harvested_segments_mean +
                  (harvested_segments_count[[i]] - previous_harvested_segments_mean) / r
                previous_harvested_segments_var_by_r <- results$harvested_segments[[i]]$sd[, tm, season]
                results$harvested_segments[[i]]$sd[, tm, season] <- previous_harvested_segments_var_by_r +
                ...
                if (r == 1) {
                  results$harvested_segments[[i]]$min[, tm, season] <- harvested_segments_count[[i]]
                  results$harvested_segments[[i]]$max[, tm, season] <- harvested_segments_count[[i]]
                } else {
                  results$harvested_segments[[i]]$min[, tm, season] <- pmin(
                    results$harvested_segments[[i]]$min[, tm, season],
                    harvested_segments_count[[i]]
                  )
                  results$harvested_segments[[i]]$max[, tm, season] <- pmax(
                    results$harvested_segments[[i]]$max[, tm, season],
                    harvested_segments_count[[i]]
                  )
                }
                previous_all_harvested_segments_mean <- results$all$harvested_segments[[i]]$mean[tm, season]
                results$all$harvested_segments[[i]]$mean[tm, season] <- previous_all_harvested_segments_mean +
                  (all_harvested_segments_count[[i]] - previous_all_harvested_segments_mean) / r
                previous_all_harvested_segments_var_by_r <- results$all$harvested_segments[[i]]$sd[tm, season]
                results$all$harvested_segments[[i]]$sd[tm, season] <- previous_all_harvested_segments_var_by_r +
                ...
                results$all$harvested_segments[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$harvested_segments[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
              ...
              results$harvested_segments[[i]][, tm, season] <- harvested_segments_count[[i]]
              results$all$harvested_segments[[i]][tm, season] <- all_harvested_segments_count[[i]]
            }
          }
        } else if (length(separated_stage_indices)) {
          for (i in length(separated_stage_indices)) {
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                results$harvested_stages[[i]][, tm, season, r] <- harvested_stages_count[[i]]
                results$all$harvested_stages[[i]][tm, season, r] <- all_harvested_stages_count[[i]]
              } else {
                previous_harvested_stages_mean <- results$harvested_stages[[i]]$mean[, tm, season]
                results$harvested_stages[[i]]$mean[, tm, season] <- previous_harvested_stages_mean +
                  (harvested_stages_count[[i]] - previous_harvested_stages_mean) / r
                previous_harvested_stages_var_by_r <- results$harvested_stages[[i]]$sd[, tm, season]
                results$harvested_stages[[i]]$sd[, tm, season] <- previous_harvested_stages_var_by_r +
                ...
                if (r == 1) {
                  results$harvested_stages[[i]]$min[, tm, season] <- harvested_stages_count[[i]]
                  results$harvested_stages[[i]]$max[, tm, season] <- harvested_stages_count[[i]]
                } else {
                  results$harvested_stages[[i]]$min[, tm, season] <- pmin(
                    results$harvested_stages[[i]]$min[, tm, season],
                    harvested_stages_count[[i]]
                  )
                  results$harvested_stages[[i]]$max[, tm, season] <- pmax(
                    results$harvested_stages[[i]]$max[, tm, season],
                    harvested_stages_count[[i]]
                  )
                }
                previous_all_harvested_stages_mean <- results$all$harvested_stages[[i]]$mean[tm, season]
                results$all$harvested_stages[[i]]$mean[tm, season] <- previous_all_harvested_stages_mean +
                  (all_harvested_stages_count[[i]] - previous_all_harvested_stages_mean) / r
                previous_all_harvested_stages_var_by_r <- results$all$harvested_stages[[i]]$sd[tm, season]
                results$all$harvested_stages[[i]]$sd[tm, season] <- previous_all_harvested_stages_var_by_r +
                ...
                results$all$harvested_stages[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$harvested_stages[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
              ...
              results$harvested_stages[[i]][, tm, season] <- harvested_stages_count[[i]]
              results$all$harvested_stages[[i]][tm, season] <- all_harvested_stages_count[[i]]
            }
          }
        } else if (length(separated_compartment_indices)) {
          for (i in 1:length(separated_compartment_indices)) {
            if (replicates > 1) {
              if ("replicate" %in% results_selection) {
                results$harvested_compartments[[i]][, tm, season, r] <- harvested_compartments_count[[i]]
                results$all$harvested_compartments[[i]][tm, season, r] <- all_harvested_compartments_count[[i]]
              } else {
                previous_harvested_compartments_mean <- results$harvested_compartments[[i]]$mean[, tm, season]
                results$harvested_compartments[[i]]$mean[, tm, season] <- previous_harvested_compartments_mean +
                  (harvested_compartments_count[[i]] - previous_harvested_compartments_mean) / r
                previous_harvested_compartments_var_by_r <- results$harvested_compartments[[i]]$sd[, tm, season]
                results$harvested_compartments[[i]]$sd[, tm, season] <- previous_harvested_compartments_var_by_r +
                ...
                if (r == 1) {
                  results$harvested_compartments[[i]]$min[, tm, season] <- harvested_compartments_count[[i]]
                  results$harvested_compartments[[i]]$max[, tm, season] <- harvested_compartments_count[[i]]
                } else {
                  ...
                  results$harvested_compartments[[i]]$max[, tm, season] <- pmax(
                    results$harvested_compartments[[i]]$max[, tm, season],
                    harvested_compartments_count[[i]]
                  )
                }
                previous_all_harvested_compartments_mean <- results$all$harvested_compartments[[i]]$mean[tm, season]
                results$all$harvested_compartments[[i]]$mean[tm, season] <- previous_all_harvested_compartments_mean +
                  (all_harvested_compartments_count[[i]] - previous_all_harvested_compartments_mean) / r
                previous_all_harvested_compartments_var_by_r <- results$all$harvested_compartments[[i]]$sd[tm, season]
                results$all$harvested_compartments[[i]]$sd[tm, season] <- previous_all_harvested_compartments_var_by_r +
                ...
                results$all$harvested_compartments[[i]]$min[tm, season] <- ifelse(r > 1,
                  min(
                ...
                results$all$harvested_compartments[[i]]$max[tm, season] <- ifelse(r > 1,
                  max(
              ...
              results$harvested_compartments[[i]][, tm, season] <- harvested_compartments_count[[i]]
              results$all$harvested_compartments[[i]][tm, season] <- all_harvested_compartments_count[[i]]
            }
          }
        }
      }
  ...
  calculate_at_replicate <- function(r, segment_abundance, results) {
    # Sum abundance counts for included segments
    all_abundance_count <- sum(.colSums(segment_abundance[segment_indices, ],
      m = result_segment_length,
      n = populations
    ))
    ...
    if ("extinction_location" %in% results_selection && !is.null(coordinates)) {
      if (all_abundance_count == 0) { # => extinct
        ...
        if (length(last_pop_indices) > 1) {
          ...
          results$all$extinction_location[r, ] <- (.colSums(coordinates[last_pop_indices, ] * abundance_weights,
            m = length(last_pop_indices), n = 2
  ...
  finalize_attributes <- function(results) {
    ...
    if (replicates > 1 && !("replicate" %in% results_selection)) {
      if ("abundance" %in% results_selection) {
        ...
        if (length(separated_stage_indices) && length(separated_compartment_indices)) {
          results$abundance_segments <- lapply(results$abundance_segments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$abundance_segments <- lapply(results$all$abundance_segments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        } else if (length(separated_stage_indices)) {
          results$abundance_stages <- lapply(results$abundance_stages, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$abundance_stages <- lapply(results$all$abundance_stages, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        } else if (length(separated_compartment_indices)) {
          results$abundance_compartments <- lapply(results$abundance_compartments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$abundance_compartments <- lapply(results$all$abundance_compartments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        }
      ...
      if ("harvested" %in% results_selection) {
        ...
        if (length(separated_stage_indices) && length(separated_compartment_indices)) {
          results$harvested_segments <- lapply(results$harvested_segments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$harvested_segments <- lapply(results$all$harvested_segments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        } else if (length(separated_stage_indices)) {
          results$harvested_stages <- lapply(results$harvested_stages, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$harvested_stages <- lapply(results$all$harvested_stages, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        } else if (length(separated_compartment_indices)) {
          results$harvested_compartments <- lapply(results$harvested_compartments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
          results$all$harvested_compartments <- lapply(results$all$harvested_compartments, function(x) {
            x$sd <- sqrt(x$sd / (replicates - 1))
            return(x)
          })
        }
