#' Helper function to check the validity of inputs to the `sir_model_summer`
#' and `sir_model_winter` functions.
#'
#' This is an internal function that checks inputs to the `disease_simulator`
#' function to make sure they are valid for a two-stage (juvenile, adult) SIR
#' model, and sets default values for needed inputs if their values are not
#' supplied. The possible inputs for this function are the same as the possible
#' inputs to the `disease_simulator` function.
#'
...
check_aspatial_sir_inputs <- function(inputs) {
  ...
  if (stages != 2) {
    cli_abort(c("The seasonal SIR functions are built for a two-stage system.",
                "x" = "You have entered {stages} stage{?s}."))
  }
  if (compartments != 3) {
    cli_abort(c("The seasonal SIR functions are built for 3 disease
                compartments.",
  ...
  if (is.null(inputs[["fecundity_mask"]])) {
    inputs[["fecundity_mask"]] <- fecundity_mask <- rep(0, segments)
  }
  ...
  if (sum(fecundity_mask) > compartments) {
    cli_abort(c("The seasonal SIR functions assume that only adults
                reproduce.",
                "x" = "Your fecundity mask indicates that juveniles reproduce."))
  }
  if (sum(transmission_mask) > stages) {
    cli_abort(c("The seasonal SIR functions assume that only susceptibles can
                be infected.",
                "x" = "Your transmission mask indicates that infected or
                recovered individuals can be infected."))
  }
  if (sum(recovery_mask) > stages) {
    cli_abort(c("The seasonal SIR functions assume that only the infected
                compartment can recover.",
                "x" = "Your recovery mask indicates that more than one
                compartment can recover."))
  }
  ...
  if (is.null(inputs[["mortality_unit"]])) {
    ...
    if (length(unit_values) == 1) {
      inputs[["mortality_unit"]] <- rep(unit_values, segments)
    }
